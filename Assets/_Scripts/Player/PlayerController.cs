...
namespace _Scripts.Controllers
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        {
            PlayerLook();
            if (input.RollIsPressed) _playerMovement = GetMoveInput();
            ...
            #if UNITY_EDITOR
            Debug.DrawRay(_playerPosition, _playerTransform.TransformDirection(_playerMovement), Color.red, 0.5f);
            #endif
        }
        ...
        {
            if (_playerMovement != Vector3.zero && !input.RollIsPressed)
            {
                var inputDirection = GetLookDirection() * GetMoveInput().z + _playerTransform.right * GetMoveInput().x;
                bear.forward = Vector3.Slerp(bear.forward, inputDirection.normalized, rotationSpeed * Time.fixedDeltaTime);
            }
            else if (input.RollIsPressed)
            {
                
                _playerTransform.forward = GetLookDirection();
            }
        ...
        {
            var initialMaxSlopeAngle = maxSlopeAngle;
            var calculatedPlayerMovement = _playerMovement;
            ...
            if (playerIsGrounded)
            {
                ...
                if (groundSlopeAngle == 0.0f)
                {
                    if (input.MoveIsPressed)
                    {
                        ...
                        Debug.DrawRay(rayOrigin, _playerTransform.TransformDirection(calculatedPlayerMovement), Color.green, 1.0f);
                    }
                ...
                else
                {
                    ...
                    // var rampAngleRotation = Quaternion.FromToRotation(playerUp, localGroundCheckHitNormal);
                    // var test = rampAngleRotation * _playerTransform.forward;
                    //
                    // var relativeRampAngle = Vector3.Angle(test, playerUp) - 90f;
                    //
                    // Debug.Log(calculatedPlayerMovement + " " + relativeSlopeAngle);
                    // Debug.Log(_playerTransform.forward + " " + relativeRampAngle);
                    
                    if (calculatedPlayerMovement != Vector3.zero) playerGoingUpRamp = !(relativeSlopeAngle > 0);
        ...
        private bool PlayerGroundCheck()
        {
            ...
            Debug.DrawRay(_playerPosition, _playerTransform.TransformDirection(Vector3.down), Color.magenta, 0.5f);
